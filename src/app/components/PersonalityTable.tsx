...
export default function PersonalityTable({ onPersonalitySelect }: PersonalityTableProps) {
  ...
  const filteredPersonalities = personalityMapping.filter(personality => {
    const riskMatch = selectedRisk === 'all' || personality.riskLevel === selectedRisk;
    const appetiteMatch = selectedAppetite === 'all' || personality.noveltyAppetite === selectedAppetite;
    const holdingMatch = selectedHoldingTime === 'all' || personality.holdingTime === selectedHoldingTime;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-6">
      <h2 className="text-3xl font-bold text-gray-900 mb-6">Personality Trading Profiles</h2>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredPersonalities.map((personality, index) => (
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getRiskColor(personality.riskLevel)}`}>
                    {personality.riskLevel}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getAppetiteColor(personality.noveltyAppetite)}`}>
                    {personality.noveltyAppetite}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getHoldingTimeColor(personality.holdingTime)}`}>
                ...
                <td className="px-6 py-4">
                  <div className="text-sm text-gray-900">
                    {personality.tokenPairs.map((pair, i) => (
                      <div key={i} className="text-xs bg-gray-100 px-2 py-1 rounded mr-1 mb-1 inline-block">
                        {pair}
                      </div>
                    ))}
                  </div>
